...
//------------ User Model ------------//
const dstudentUser = require('../src/models/studentUser');
...
exports.registerHandle = (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        //------------ Validation passed ------------//
        dstudentUser.findOne({ email: email }).then(user => {
            if (user) {
                //------------ User already exists ------------//
            ...
            if (err) {
                ...
                dstudentUser.findOne({ email: email }).then(user => {
                    if (user) {
                        ...
                        const newUser = new dstudentUser({
                            name,
                            email,
                            password
                        });
    ...
    if (errors.length > 0) {
        ...
        dstudentUser.findOne({ email: email }).then(user => {
            if (!user) {
                ...
                dstudentUser.updateOne({ resetLink: token }, (err, success) => {
                    if (err) {
    ...
    if (token) {
        jwt.verify(token, JWT_RESET_KEY, (err, decodedToken) => {
            if (err) {
                ...
                dstudentUser.findById(_id, (err, user) => {
                    if (err) {
                        req.flash(
                            'error_msg',
    ...
    //------------ Checking password mismatch ------------//
        ...
        bcryptjs.genSalt(10, (err, salt) => {
            bcryptjs.hash(password, salt, (err, hash) => {
                ...
                dstudentUser.findByIdAndUpdate(
                    { _id: id },
                    { password },
                    function (err, result) {
                        if (err) {
